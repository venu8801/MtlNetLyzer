...
char *filter_extraction(int argc, char *argv[])
{
    ...
    struct
    {
        ...
        {"assoc-req", 0}, {"assoc-resp", 1}, {"reassoc-req", 2}, {"reassoc-resp", 3}, {"probe-req", 4}, {"probe-resp", 5}, {"beacon", 8}, {"atim", 9}, {"disassoc", 10}, {"auth", 11}, {"deauth", 12}, {"action", 13}, {NULL, -1}};
    ...
    struct
    {
        ...
        {"block-ack-req", 8}, {"block-ack", 9}, {"ps-poll", 10}, {"rts", 11}, {"cts", 12}, {"ack", 13}, {"cf-end", 14}, {"cf-end-ack", 15}, {NULL, -1}
    ...
    struct
    {
        ...
        {"data", 0}, {"data-cf-ack", 1}, {"data-cf-poll", 2}, {"data-cf-ack-poll", 3}, {"null", 4}, {"cf-ack", 5}, {"cf-poll", 6}, {"cf-ack-poll", 7}, {"qos-data", 8}, {"qos-data-cf-ack", 9}, {"qos-data-cf-poll", 10}, {"qos-data-cf-ack-poll", 11}, {"qos-null", 12}, {"qos-cf-poll", 14}, {"qos-cf-ack-poll", 15}, {NULL, -1}
    ...
    char *filter_exp = (char *)malloc(1024 * sizeof(char)); /*allocating memory dynamically because filter_exp need to return to the main function
    ...
    if ((frame_type != NULL) && strcmp(frame_type, "ctl") == 0)
    {
        if (subtype_str)
        {
            if (mac_address)
            {
                snprintf(filter_exp, sizeof(buffer_size), "wlan type ctl subtype %s and (wlan addr1 %s or wlan addr2 %s)", subtype_str, mac_address, mac_address);
            }
            else
            {
        ...
        else
        {
            if (mac_address)
            {
                snprintf(filter_exp, sizeof(buffer_size), "wlan type ctl and (wlan addr1 %s or wlan addr2 %s)", mac_address, mac_address);
            }
            else
            {
                snprintf(filter_exp, sizeof(buffer_size), "wlan type ctl");
            }
        }
    }
    else if (frame_type != NULL)
    {
        if (subtype_str)
        {
            if (mac_address)
            {
                snprintf(filter_exp, sizeof(buffer_size), "wlan type %s subtype %s and ether host %s", frame_type, subtype_str, mac_address);
            }
            else
            {
                snprintf(filter_exp, sizeof(buffer_size), " wlan type %s subtype %s", frame_type, subtype_str);
        ...
        else
        {
            if (mac_address)
            {
                snprintf(filter_exp, sizeof(buffer_size), "ether host %s", mac_address);
            }
            else
            {
    ...
    else if (frame_type == NULL && mac_address)
    {
        snprintf(filter_exp, sizeof(buffer_size), "ether host %s", mac_address);
    }
    else
